...
import { Send, Hash, Users, Circle, ArrowLeft } from 'lucide-react';
...
export default function TitleChat() {
    ...
    useEffect(() => {
        const fetchHistory = async () => {
            try {
                const res = await API.get(`/api/chat/${roomName}`);
            ...
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', background: 'var(--bg-elevated)', borderRadius: 'var(--radius-xl)', border: '1px solid var(--border-dim)', overflow: 'hidden', minWidth: 0 }}>
                ...
                <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--border-dim)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0, background: 'var(--bg-card)' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                        <Link to={`/manhwa/${id}`} style={{ color: 'var(--text-muted)' }}><ArrowLeft size={20} /></Link>
                        {title && <img src={title.coverImage?.large} alt="" style={{ width: 32, height: 44, borderRadius: 4, objectFit: 'cover' }} />}
                        <div>
                            <div style={{ fontWeight: 700, fontFamily: 'Space Grotesk, sans-serif', fontSize: '15px' }}>{title?.title?.english || title?.title?.romaji || 'Loading...'}</div>
                            <div style={{ fontSize: '11px', color: 'var(--text-muted)' }}>Spoiler-free zone • Title Discussion</div>
                        </div>
                    </div>
                ...
                <div style={{ padding: '14px 20px', borderTop: '1px solid var(--border-dim)', background: 'var(--bg-card)', flexShrink: 0 }}>
                    <form onSubmit={send} style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
                        <input
                            ...
                            placeholder={user ? `Message #${roomName}-pub...` : 'Log in to chat...'}
                            disabled={!user}
                        />
