...
import { Edit3, Activity, BarChart2, Loader2, X, UserCheck } from 'lucide-react';
import { updateProfile, getProfile } from '../api/apiClient';
...
export default function Profile() {
  ...
  if (pageError || !profileData) {
    return (
      <div style={{ textAlign: 'center', paddingTop: 80, color: 'var(--crimson-warm)' }}>
        {pageError || 'Profile not found.'}
      </div>
    );
  }

  const { user, reviews = [], libraryStats = {}, isFriend } = profileData;
      ...
      <div style={{
        ...
        {isOwnProfile && (
          <div style={{ position: 'absolute', top: 16, right: 16, display: 'flex', gap: 10, zIndex: 20 }}>
            ...
            >
              {saving ? <Loader2 size={14} className="animate-spin" /> : isEditing ? 'Save Profile' : 'Edit Profile'}
            </button>
          </div>
        )}

        {!isOwnProfile && isFriend && (
          <div style={{ position: 'absolute', top: 16, right: 16, zIndex: 20, background: 'rgba(0,0,0,0.5)', border: '1px solid var(--border-muted)', borderRadius: 'var(--radius-md)', padding: '8px 14px', display: 'flex', alignItems: 'center', gap: 6, backdropFilter: 'blur(12px)', color: 'var(--green-neon)', fontSize: '13px', fontWeight: 600 }}>
      ...
      <div style={{ background: 'var(--bg-elevated)', border: '1px solid var(--border-dim)', borderRadius: '0 0 var(--radius-xl) var(--radius-xl)', padding: '0 clamp(16px, 3vw, 32px) 24px', marginBottom: '24px' }}>
        <div style={{ display: 'flex', alignItems: 'flex-end', gap: 'clamp(12px, 3vw, 24px)', transform: 'translateY(-40px)', marginBottom: '-16px' }}>
          <div style={{ position: 'relative', flexShrink: 0 }}>
            <img
              src={isEditing && isOwnProfile ? (form.avatar || `https://api.dicebear.com/7.x/avataaars/svg?seed=${form.username}`) : getAvatarUrl(user.avatar, user.username)}
              style={{ width: 'clamp(80px, 10vw, 120px)', height: 'clamp(80px, 10vw, 120px)', borderRadius: '50%', border: '4px solid var(--bg-elevated)', objectFit: 'cover', background: 'var(--bg-card)', boxShadow: 'var(--shadow-blue)' }}
              alt="profile picture"
            />
            {isEditing && isOwnProfile && (
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'clamp(200px, 28%, 260px) minmax(0,1fr)', gap: '20px' }} className="profile-grid">
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          ...
          <div style={{ background: 'var(--bg-elevated)', border: '1px solid var(--border-dim)', borderRadius: 'var(--radius-lg)', padding: '20px' }}>
            ...
            {[
              ['Total Read', libraryStats.totalRead ?? 0, 'var(--blue-neon)'],
              ['Reviews', reviews.length ?? 0, 'var(--gold-warm)'],
              ['Friends', libraryStats.friendsCount ?? 0, 'var(--green-neon)'],
              ['Karma', user.karma || 0, 'var(--crimson-warm)']
            ].map(([k, v, c]) => (
